...
class LusiCompositePeaks:

    def __init__(self, peaks = None):

        super(LusiCompositePeaks, self).__init__()
    ...
    def get_composite_bounds(self, start, end):
        ...
        for i in range(end - start + 1):
            ...
            heartbeat_time   = self.peaks.time[interval] - self.peaks.time[self.peaks.dT.data[start + 1 + i]]
    ...
    def get_clipped_heartbeat_signals(self, i, start, interval, composite_start, composite_end):
        ...
        heartbeat_time   = self.peaks.time[interval]   - self.peaks.time[self.peaks.dT.data[start + 1 + i]]
        heartbeat_signal = self.peaks.signal[interval] - self.peaks.signal[self.peaks.dT.data[start + 1 + i]]
        heartbeat_seis   = self.peaks.seis[interval]   - self.peaks.signal[self.peaks.dT.data[start + 1 + i]]
        heartbeat_phono  = self.peaks.phono[interval]  - self.peaks.signal[self.peaks.dT.data[start + 1 + i]]
    ...
    def get_N_composite_signal_dataset(self, N, slide_step_size, display = False, dosage = None):
        ...
        for start, end in composite_endpoints:
            ...
            if display:
                ...
                if dosage is None:
                    plt.suptitle(str(count) + " of " + str(len(composite_endpoints)) + " LUSI Composites given " + str(N) + " Heartbeats w/ Step Size of " + str(slide_step_size) , fontsize=20)
                else:
                    plt.suptitle(str(count) + " of " + str(len(composite_endpoints)) + " LUSI Composites for Dosage " + str(dosage) + " given " + str(N) + " Heartbeats w/ Step Size of " + str(slide_step_size) , fontsize=20)
                ...
                for i, row in enumerate(axes2d):
                    for j, cell in enumerate(row):
                        ...
                        if j == 0:
                            ...
                            signal_lay_over_cell.plot(self.peaks.time[interval] - self.peaks.time[self.peaks.dT.data[start + 1 + i]], hb.normalize(self.peaks.signal[interval] - self.peaks.signal[self.peaks.dT.data[start + 1 + i]]), "--", linewidth= 0.5)
                            seis_lay_over_cell.plot(self.peaks.time[interval]   - self.peaks.time[self.peaks.dT.data[start + 1 + i]], hb.normalize(self.peaks.seis[interval]   - self.peaks.signal[self.peaks.dT.data[start + 1 + i]]), "--", linewidth= 0.5)
                            phono_lay_over_cell.plot(self.peaks.time[interval]  - self.peaks.time[self.peaks.dT.data[start + 1 + i]], hb.normalize(self.peaks.phono[interval]  - self.peaks.signal[self.peaks.dT.data[start + 1 + i]]), "--", linewidth= 0.5)
    ...
    def get_composite_R_peak(self, signal):
        first, _ = hb.get_derivatives(signal)
        area = range(int(np.argmax(-first) - 0.1*len(signal)), int(np.argmax(-first) + 0.1*len(signal)))

        self.R.data.append(int(np.argmax(-first) - 0.1*len(signal) + np.argmax(signal[area])))
    ...
    def add_composite_T_peak(self, index, signal, t_window_ratio):
        ...
        # Add T peak
        self.T.data.append(int(self.S.data[index] + np.argmax(signal[t_window])))
